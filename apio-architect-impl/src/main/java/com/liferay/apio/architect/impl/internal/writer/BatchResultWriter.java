...
public class BatchResultWriter<T> {
	...
	public Optional<String> write() {
		...
		_batchResultMessageMapper.onStart(
			_jsonObjectBuilder, _batchResult, _requestInfo.getHttpHeaders());
		...
		for (T identifier : identifiers) {
			...
			_pathFunction.apply(
				...
				path -> {
					...
					String url = createSingleURL(serverURL, path);
		...
		_batchResultMessageMapper.onFinish(
			_jsonObjectBuilder, _batchResult, _requestInfo.getHttpHeaders());
